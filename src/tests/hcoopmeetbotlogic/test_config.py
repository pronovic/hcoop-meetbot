...
MISSING_DIR = "bogus"
VALID_DIR = str(Path(__file__).parent / "fixtures/test_config/valid")  # valid config with no optional values
OPTIONAL_DIR = str(Path(__file__).parent / "fixtures/test_config/optional")  # valid config with optional values
NO_CHANNEL_DIR = str(Path(__file__).parent / "fixtures/test_config/nochannel")
NO_FILE_DIR = str(Path(__file__).parent / "fixtures/test_config/nofile")
EMPTY_DIR = str(Path(__file__).parent / "fixtures/test_config/empty")
INVALID_DIR = str(Path(__file__).parent / "fixtures/test_config/invalid")
BAD_BOOLEAN_DIR = str(Path(__file__).parent / "fixtures/test_config/bad_boolean")
BAD_FORMAT_DIR = str(Path(__file__).parent / "fixtures/test_config/bad_format")
...
class TestParsing:
    def test_valid_configuration_dir(self):
        logger = MagicMock()
        conf_dir = VALID_DIR
        conf_file = str(Path(conf_dir) / "HcoopMeetbot.conf")  # if the caller provides a directory, we always load this file
    ...
    def test_valid_configuration_file(self):
        logger = MagicMock()
        conf_dir = VALID_DIR
        conf_file = str(Path(conf_dir) / "CustomName.conf")  # for anything other than a directory, we open it like a file
        assert Path(conf_dir).is_dir() and Path(conf_file).is_file()
    ...
    def test_missing_configuration_file_default(self):
        ...
        conf_file = str(Path(conf_dir) / "HcoopMeetbot.conf")
        assert Path(conf_dir).exists() and not Path(conf_file).exists()
    ...
    def test_missing_configuration_file_custom(self):
        logger = MagicMock()
        conf_dir = NO_FILE_DIR
        conf_file = str(Path(conf_dir) / "AnyFile.conf")
        assert Path(conf_dir).exists() and not Path(conf_file).exists()
